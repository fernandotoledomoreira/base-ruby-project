...
pipeline {
    ...
    stages {
        ...
        stage('Check Branch Name') {
            steps {
                script {
                    if (BRANCH_NAME.startsWith("master") || BRANCH_NAME.startsWith("develop")) {
        ...
        stage('Pre-Build') {
            ...
            steps {
                script {
                    ...
                    env['url_docker'] = "758526784474.dkr.ecr.us-east-1.amazonaws.com/base-ruby-backend-qa:${newVersion}"
                }
            ...
            steps {
                script {
                    ...
                    docker.withRegistry("${env.ECR_URL}",'ecr:us-east-1:ecr-private-registry'){
                        def app = docker.build("base-ruby-backend-qa")
        ...
        stage ('delete image'){
            ...
            steps{
                script{
                    removeImage('base-ruby-backend-qa',env['newVersion'])
                }
            }
        }
...
def notifyBuild(String buildStatus = 'STARTED') {
    ...
    if (buildStatus == 'STARTED') {
        ...
        field.put('value', 'base-ruby-backend-qa');
        fields.add(field);
